...
## Mockバイアスの破棄 (2026-09-25)
- Mockオッズを使った検証結果は全て無効
- truncationバグ前の予想確率も無効
- JRA 5538レースのバックテストはオッズが壊れていたため無効
- 混在モードの閾値（EV>=0, o>=30, N=2）は根拠なし
- 実データ取得完了後、ゼロから再導出する

## 現在の実データ源
- 予想・検証は NAR 過去データ (oddspark) のみで行う
- JRAは netkeiba API の type マッピングが未修正のため当面使わない
- NAR過去データは bulk_fetch_nar.py で取得
